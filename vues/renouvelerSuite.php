...
| -------------------------
| DESCRIPTION
|   VUE - Affichage de la suite de renouvellement
|------------------------------------- */
...
if(isset($_POST['noMembre']) && !empty($_POST["noMembre"]) && preg_match('/[0-9]+/',$_POST["noMembre"])) {
    $noMembre = $_POST["noMembre"];
    while(substr($noMembre,0,1) == "0") {
        $noMembreLen = strlen($noMembre)-1;
        $noMembre = substr($noMembre,1,$noMembreLen);
    }
} else {
    $noMembre =NULL;
}
...
if(count($err) > 0) {
    header("location:renouveler.html?&err&noMembre=$noMembre&typeMembre=$typeMembre&prenom=$prenom&nom=$nom&tel1=$tel1&tel2=$tel2&tel3=$tel3&cell1=$cell1&cell2=$cell2&cell3=$cell3&courriel=$courriel&codePostal=$codePostal&adresse=$adresse&ville=$ville&typeCommunication=$typeCommunication");
}
...
<body>
    <div class="page-wrap">
        ...
        <h2>Formulaire de Renouvellement</h2>
            ...
            <p id="noMembre"> <span>No de Membre :</span> <?= $noMembre ?></p>
