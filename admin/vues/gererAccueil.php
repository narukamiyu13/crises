...
| -------------------------
| DESCRIPTION
|   VUE : la page de gestion du contenu de la page d'accueil
|------------------------------------- */
$msg = "&nbsp;";
...
if(isset($_POST['updateAccueil'])){
    ...
    $texte = $_POST['contenuAccueil'];
        ...
        $this->modele->setTexteAccueil($texte);
    header("location:gererAccueil.html?ok");
...
$remplacements = array('Janvier', 'Février','Mars', 'Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre');
    ...
    $date = utf8_encode(strftime("%e %B %Y %R", strtotime($this->modele->getTexteAccueil()['derniereModification'])));
...
<body id="dashboard-parametres" class='dashboard gererServices'>
    ...
    <?php if($_SESSION['permission'] <= 3) { ?>
        <p style="margin-bottom:20px;text-align:left;">Dernière Modification: le <?=$date ?> par <?=$this->modele->getTexteAccueil()['prenomAdmin']; ?> <?=$this->modele->getTexteAccueil()['nomAdmin']; ?></p>
        <?php } ?>
    <form method="post" action="gererAccueil.html">
        
    <textarea name="contenuAccueil"><?=$this->modele->getTexteAccueil()['texteAccueil']; ?></textarea>
        <input type="submit" name="updateAccueil" value="Accepter" />
    </form>
</body>
</html>