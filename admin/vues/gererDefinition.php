...
| -------------------------
| DESCRIPTION
|   VUE : la page de gestion du contenu de la page définition d'une crise
|------------------------------------- */
...
if(isset($_POST['updateDefinition'])){
    try {
    $texte = $_POST['contenuAccueil'];
    
        $this->modele->setTexteDefinition($texte);
    header("location:gererDefinition.html?ok");
...
$date = utf8_encode(strftime("%e %B %Y %R", strtotime($this->modele->getTexteDefinition()['derniereModification'])));
...
<body id="dashboard-parametres" class='dashboard gererServices'>
    ...
    <h2>Modifier la page Définition d'une<br>crise</h2>
    <p style="margin-bottom:50px;"><?=$msg; ?></p>
    <?php if($_SESSION['permission'] <= 3) { ?>
        <p style="margin-bottom:20px;text-align:left;">Dernière Modification: le <?= $date ?> par <?=$this->modele->getTexteDefinition()['prenomAdmin']; ?> <?=$this->modele->getTexteDefinition()['nomAdmin']; ?></p>
        <?php } ?>
    <form method="post" action="gererDefinition.html">
        
    <textarea name="contenuAccueil"><?=$this->modele->getTexteDefinition()['texteDefinition']; ?></textarea>
        <input type="submit" name="updateDefinition" value="Accepter" />
    </form>
</body>
</html>