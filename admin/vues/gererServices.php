...
| -------------------------
| DESCRIPTION
|   VUE : la page de gestion du contenu de la page Services
|------------------------------------- */
...
if(isset($_POST['updateServices'])){
    try {
    $texte = $_POST['contenuServices'];
    
        $this->modele->setTexteServices($texte);
    header("location:gererServices.html?ok");
...
$date = utf8_encode(strftime("%e %B %Y %R", strtotime($this->modele->getTexteServices()['derniereModification'])));
...
<body id="dashboard-parametres" class='dashboard gererServices'>
    ...
    <h2>Contenu de la page Services</h2>
    <p style="margin-bottom:50px;"><?=$msg; ?></p>
    <?php if($_SESSION['permission'] <= 3) { ?>
        <p style="margin-bottom:20px;text-align:left;">Dernière Modification: le <?=$date ?> par <?=$this->modele->getTexteServices()['prenomAdmin']; ?> <?=$this->modele->getTexteServices()['nomAdmin']; ?></p>
        <?php } ?>
    <form method="post" action="gererServices.html">
        
    <textarea name="contenuServices"><?=$this->modele->getTexteServices()['texteServices']; ?></textarea>
        <input type="submit" name="updateServices" value="Accepter" />
    </form>
</body>
</html>