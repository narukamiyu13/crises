...
| -------------------------
| DESCRIPTION
|   VUE : la page de gestion du contenu de la page Philosphie
|------------------------------------- */
...
if(isset($_POST['updatePhilosophie'])){
    try {
    $texte = $_POST['contenuAccueil'];
    
        $this->modele->setTextePhilosophie($texte);
    header("location:gererPhilosophie.html?ok");
...
$date = utf8_encode(strftime("%e %B %Y %R", strtotime($this->modele->getTextePhilosophie()['derniereModification'])));
...
<body id="dashboard-parametres" class='dashboard gererServices'>
    ...
    <h2>Modifier la page Philosophie</h2>
    <p style="margin-bottom:50px;"><?=$msg; ?></p>
    <?php if($_SESSION['permission'] <= 3) { ?>
        <p style="margin-bottom:20px;text-align:left;">Dernière Modification: le <?=$date?> par <?=$this->modele->getTextePhilosophie()['prenomAdmin']; ?> <?=$this->modele->getTextePhilosophie()['nomAdmin']; ?></p>
        <?php } ?>
    <form method="post" action="gererPhilosophie.html">
        
    <textarea name="contenuAccueil"><?=$this->modele->getTextePhilosophie()['textePhilosophie']; ?></textarea>
        <input type="submit" name="updatePhilosophie" value="Accepter" />
    </form>
</body>
</html>