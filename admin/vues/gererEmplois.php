...
| -------------------------
| DESCRIPTION
|   VUE : la page de gestion du contenu de la page emplois
|------------------------------------- */
...
if(isset($_POST['updateAdmissibilite'])){
    ...
    $texte = $_POST['contenuAccueil'];
        ...
        $this->modele->setTexteEmploi($texte);
    header("location:gererEmplois.html?ok");
...
$date = utf8_encode(strftime("%e %B %Y %R", strtotime($this->modele->getTexteEmploi()['derniereModification'])));
...
<body id="dashboard-parametres" class='dashboard gererServices'>
    ...
    <h2>Modifier la page Emplois</h2>
    <p style="margin-bottom:50px;"><?=$msg; ?></p>
    <?php if($_SESSION['permission'] <= 3) { ?>
        <p style="margin-bottom:20px;text-align:left;">Dernière Modification: le <?= $date?> par <?=$this->modele->getTexteEmploi()['prenomAdmin']; ?> <?=$this->modele->getTexteEmploi()['nomAdmin']; ?></p>
        <?php } ?>
    <form method="post" action="gererEmplois.html">
        
    <textarea name="contenuAccueil"><?=$this->modele->getTexteEmploi()['texteEmplois']; ?></textarea>
